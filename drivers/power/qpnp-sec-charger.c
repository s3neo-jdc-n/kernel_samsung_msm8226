...
struct qpnp_chg_chip {
	...
	struct hrtimer			reduce_power_stage_alarm;
	...
	struct hrtimer			event_termination_alarm;
 ...
 *
 */
//static void sec_bat_event_expired_timer_func(struct hrtimer *alarm);
 ...
 * status accordingly
 */
//static void sec_bat_polling_alarm_expired(struct hrtimer *alarm);
...
qpnp_chg_reduce_power_stage(struct qpnp_chg_chip *chip)
{
	...
	if (usb_present && usb_ma_above_wall) {
		...
		hrtimer_start_range_ns(&chip->reduce_power_stage_alarm,
...
/**static void
qpnp_chg_reduce_power_stage_callback(struct hrtimer *alarm)
{
        ...
        struct qpnp_chg_chip *chip, int seconds)
{
        ...
        hrtimer_start_range_ns(&chip->event_termination_alarm,
...
static void sec_bat_program_alarm(struct qpnp_chg_chip *chip, int polling_time)
{
	...
	hrtimer_start_range_ns(&chip->polling_alarm,
		next, ULONG_MAX, HRTIMER_MODE_ABS);
}
...
qpnp_charger_probe(struct spmi_device *spmi)
{
	...
	hrtimer_init(&chip->reduce_power_stage_alarm, CLOCK_BOOTTIME, HRTIMER_MODE_ABS);
	...
	hrtimer_init(&chip->event_termination_alarm, CLOCK_BOOTTIME, HRTIMER_MODE_ABS);
	hrtimer_init(&chip->polling_alarm, CLOCK_BOOTTIME, HRTIMER_MODE_ABS);
...
static int sec_qpnp_chg_prepare(struct device *dev)
{
	...
	hrtimer_cancel(&chip->polling_alarm);
